...
window.addEventListener('pywebviewready', () => {
  ...
  // Check for updates silently — runs 4s after launch so startup isn't slowed
  setTimeout(_checkForUpdate, 4000);
});
...
async function _checkForUpdate() {
  try {
    ...
    if (!r || r.status !== 'ok' || !r.update_available) return;

    // Don't show banner if user already dismissed this version this session
    ...
    if (notes && r.release_notes)
      notes.textContent = r.release_notes.split('\n')[0] || '';

    _updateUrl = r.release_url;
    banner.style.display = 'flex';

    // Push app content down so banner doesn't cover anything
    const shell = document.querySelector('.app-shell');
    if (shell) shell.style.marginTop = banner.offsetHeight + 'px';

  } catch(e) {
    // Silently ignore — no internet, private network, etc.
  }
}

function downloadUpdate() {
  if (_updateUrl) window.pywebview.api.open_url(_updateUrl);
}
...
function dismissUpdate() {
  ...
  try {
    ...
    const match = msg.match(/v([\d.]+) is available/);
    if (match) sessionStorage.setItem('update_dismissed', match[1]);
  } catch(e) {}
}
